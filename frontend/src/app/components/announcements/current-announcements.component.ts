...
export class CurrentAnnouncementsComponent implements OnInit {
  ...
  public constructor(
    ...
    private _service: CurrentAnnouncementsService,
  ...
  public ngOnInit(): void {
    this._service.get().subscribe((resp: ICurrentAnnouncements | null) => {
      if (resp) {
        let title = resp.title;
        if ((title?.length ?? 0) === 0) {
          title = this._translationService.get('Announcements/AnnouncementsOfTheDay', {
            date: (new GdatePipe(this._cultureService, this._translationService).transform(resp.validFromDate)),
          });
        }
        ...
        this.showButtonAddAnnouncement.set(this._permissionService.hasPermission(SystemPermissionValue.AddAnnouncements));
      }
    });
  }
}
