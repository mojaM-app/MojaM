...
import { ISettingsForm } from './settings.form';
...
export class SettingsComponent extends WithForm<ISettingsForm>() {
  protected readonly showSaveButton: WritableSignal<boolean> = signal<boolean>(false);
  private readonly _element = viewChild.required('darkModeSwitch', { read: ElementRef });
  private readonly _darkModeChanged: Signal<boolean>;
  private readonly _fontSizeChanged: Signal<number>;
  ...
  public constructor(
    ...
    this.formGroup.patchValue({
      ...
      fontSize: fontSizeService.getCurrentFontSize(),
    });

    this._darkModeChanged = toSignal(this.controls.isDarkMode.valueChanges, {
      initialValue: themeService.isDarkMode(),
    });

    this._fontSizeChanged = toSignal(this.controls.fontSize.valueChanges, {
      initialValue: fontSizeService.getCurrentFontSize(),
    });

    effect(() => {
      themeService.onOffDarkMode(this._darkModeChanged());
      fontSizeService.setFontSize(this._fontSizeChanged());
      ...
      if (this._element()?.nativeElement) {
        ...
        setTimeout(() => {
          switchElement
            ...
            ?.firstChild.setAttribute('d', this._moon);
          switchElement
            .querySelector('.mdc-switch__icon--off')
            ?.firstChild.setAttribute('d', this._sun);
        }, 100);
      }
    });

    effect(() => {
      this.showSaveButton.set(authService.isAuthenticated());
    });
  }

  protected formatFontSizeLabel(value: number): string {
    return `${value}%`;
  }
}
