...
export class CreateTableAnnouncements1729890701959 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE \`announcements\` (
        ...
        UNIQUE INDEX \`UQ_Announcement_Uuid\` (\`Uuid\`),
        PRIMARY KEY (\`Id\`)
      ) ENGINE=InnoDB
    `);
    ...
    await queryRunner.query(`
      ...
      ADD CONSTRAINT \`FK_Announcement_CreatedById_To_User\`
    ...
    await queryRunner.query(`
      ALTER TABLE \`announcements\`
      ADD CONSTRAINT \`FK_Announcement_PublishedById_To_User\`
    ...
    await queryRunner.query(`
      ...
      ADD CONSTRAINT \`FK_AnnouncementItem_CreatedById_To_User\`
      FOREIGN KEY (\`CreatedById\`) REFERENCES \`users\`(\`Id\`)
      ON DELETE RESTRICT ON UPDATE RESTRICT
    `);
    ...
    await queryRunner.query(`
      ALTER TABLE \`announcement_items\`
      ADD CONSTRAINT \`FK_AnnouncementItem_UpdatedById_To_User\`
    ...
    await queryRunner.query(`
      ALTER TABLE \`announcement_items\`
      ADD CONSTRAINT \`FK_Announcement_To_AnnouncementItem\`
      FOREIGN KEY (\`AnnouncementId\`) REFERENCES \`announcements\`(\`Id\`)
      ON DELETE RESTRICT ON UPDATE RESTRICT
    `);

    // Add permissions for announcements administration
    ...
    await queryRunner.query(
      'INSERT INTO `system_permissions` (`Id`, `Name`, `Description`, `ParentId`) VALUES (?, ?, ?, ?)',
      [SystemPermissions.AddAnnouncements, 'AddAnnouncements', 'Permission that allows add announcements', 30],
    );

    await queryRunner.query(
      'INSERT INTO `system_permissions` (`Id`, `Name`, `Description`, `ParentId`) VALUES (?, ?, ?, ?)',
      [SystemPermissions.EditAnnouncements, 'EditAnnouncements', 'Permission that allows edit announcements', 30],
    );

    await queryRunner.query(
      'INSERT INTO `system_permissions` (`Id`, `Name`, `Description`, `ParentId`) VALUES (?, ?, ?, ?)',
      [SystemPermissions.DeleteAnnouncements, 'DeleteAnnouncements', 'Permission that allows delete announcements', 30],
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `announcement_items` DROP FOREIGN KEY `FK_AnnouncementItem_UpdatedById_To_User`',
    );

    await queryRunner.query(
      'ALTER TABLE `announcement_items` DROP FOREIGN KEY `FK_AnnouncementItem_CreatedById_To_User`',
    );
