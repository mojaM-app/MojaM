...
export class Initial1729350279713 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('ALTER TABLE `user_reset_password_tokens` ADD CONSTRAINT `FK_UserResetPasswordToken_To_User_UserId` FOREIGN KEY (`UserId`) REFERENCES `users`(`Id`) ON DELETE RESTRICT ON UPDATE RESTRICT');
    ...
    await queryRunner.query('INSERT INTO `typeorm_metadata`(`database`, `schema`, `table`, `type`, `name`, `value`) VALUES (DEFAULT, DEFAULT, DEFAULT, ?, ?, ?)', ['VIEW', 'vUser', 'SELECT `user`.`Uuid` AS `Id`, `user`.`FirstName` AS `FirstName`, `user`.`LastName` AS `LastName`, `user`.`Email` AS `Email`, `user`.`Phone` AS `Phone`, `user`.`JoiningDate` AS `JoiningDate`, `user`.`LastLoginAt` AS `LastLoginAt`, `user`.`IsActive` AS `IsActive`, `user`.`IsLockedOut` AS `IsLockedOut`, `user`.`IsDeleted` AS `IsDeleted`, (select count(0) from user_to_systempermissions as perm where `user`.`Id` = perm.UserId) AS `RolesCount` FROM `users` `user`']);
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('DELETE FROM `typeorm_metadata` WHERE `type` = ? AND `name` = ?', ['VIEW', 'vUser']);
