...
export class Initial1729350279713 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'CREATE TABLE `system_permissions` (`Id` int NOT NULL, `ParentId` int NULL, `Name` varchar(50) NOT NULL, `Description` varchar(255) NOT NULL, PRIMARY KEY (`Id`)) ENGINE=InnoDB',
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `user_to_systempermissions` ADD CONSTRAINT `FK_UserSystemPermission_UserId_To_User` FOREIGN KEY (`UserId`) REFERENCES `users`(`Id`) ON DELETE RESTRICT ON UPDATE RESTRICT',
    );
    await queryRunner.query(
      'ALTER TABLE `user_to_systempermissions` ADD CONSTRAINT `FK_UserSystemPermission_To_SystemPermission` FOREIGN KEY (`PermissionId`) REFERENCES `system_permissions`(`Id`) ON DELETE RESTRICT ON UPDATE RESTRICT',
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'CREATE VIEW `vUsers` AS SELECT `user`.`Uuid` AS `Id`, `user`.`FirstName` AS `FirstName`, `user`.`LastName` AS `LastName`, `user`.`Email` AS `Email`, `user`.`Phone` AS `Phone`, `user`.`JoiningDate` AS `JoiningDate`, `user`.`LastLoginAt` AS `LastLoginAt`, `user`.`IsActive` AS `IsActive`, `user`.`IsLockedOut` AS `IsLockedOut`, `user`.`IsDeleted` AS `IsDeleted`, (select count(0) from user_to_systempermissions as perm where `user`.`Id` = perm.UserId) AS `PermissionCount` FROM `users` `user`',
    );
    await queryRunner.query(
