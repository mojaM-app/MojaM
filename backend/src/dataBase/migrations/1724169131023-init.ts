...
export class Init1724169131023 implements MigrationInterface {
  name = 'Init1724169131023'

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query('CREATE TABLE `users` (`Id` int NOT NULL AUTO_INCREMENT, `Uuid` varchar(36) NOT NULL, `Email` varchar(320) NOT NULL, `EmailConfirmed` tinyint NOT NULL DEFAULT 0, `Phone` varchar(30) NOT NULL, `PhoneConfirmed` tinyint NOT NULL DEFAULT 0, `Password` varchar(1024) NOT NULL, `Salt` varchar(64) NOT NULL, `RefreshTokenKey` varchar(128) NOT NULL, `FirstName` varchar(255) NULL, `LastName` varchar(255) NULL, `BirthDay` date NULL, `JoiningYear` date NULL, `IsActive` tinyint NOT NULL DEFAULT 0, `CreatedAt` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP, `UpdatedAt` timestamp(0) NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, `LastLoginAt` timestamp NULL, `FailedLoginAttempts` int NOT NULL DEFAULT \'0\', `IsLockedOut` tinyint NOT NULL DEFAULT 0, UNIQUE INDEX `UQ_User_Uuid` (`Uuid`), UNIQUE INDEX `UQ_User_Email_Phone` (`Email`, `Phone`), PRIMARY KEY (`Id`)) ENGINE=InnoDB');
