...
import { SystemPermission } from '../enums/system-permission.enum';
...
export class PermissionsCacheService {
  ...
  public async readAsync(userId: number): Promise<SystemPermission[] | undefined> {
    if (userId > 0) {
  ...
  public async saveAsync(userId: number, permissions: SystemPermission[]): Promise<void> {
    if (userId > 0) {
      const cacheKey = this.getIdCacheKey(userId);
  ...
  private async getDataFromCache(keyName: string): Promise<SystemPermission[] | undefined> {
    return await this.cache.getItem<SystemPermission[] | undefined>(keyName);
  }

  private async saveDataInCache(
    keyName: string,
    data: SystemPermission[] | undefined,
